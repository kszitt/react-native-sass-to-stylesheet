let options = {},
  media = {};

function getAdaptation(num, important){
  if(!options.adaptation ||
    /getAdaptation/.test(num) ||
    parseFloat(num) === 0 ||
    important) return num;

  num = parseFloat(num);

  return `getAdaptation(${num})`;
}


// Processing sass text
function CssToStyles(css){
  if(!options.space){
    options = require("../index").getOptions();
  }

  // margin, padding
  css = css.replace(/ *(margin|padding)\s*:.+?;/g, text => {
    let margin = text.match(/(margin|padding)/)[0],
      important = /!important/.test(text) ? "!important" : "",
      s = text.match(/^ */),
      array = text.replace(/ *!important/, "")
        .match(/-?[\d.]+[a-z]*/g);
    s = s ? s[0] : "";

    switch(array.length){
      case 1:
        return `${s}${margin}: ${getAdaptation(array[0], important)}${important};`;
      case 2:
        return  `${s}${margin}-top: ${getAdaptation(array[0], important)}${important};\n` +
          `${s}${margin}-bottom: ${getAdaptation(array[0], important)}${important};\n` +
          `${s}${margin}-right: ${getAdaptation(array[1], important)}${important};\n` +
          `${s}${margin}-left: ${getAdaptation(array[1], important)}${important};`;
      case 3:
        return  `${s}${margin}-top: ${getAdaptation(array[0], important)}${important};\n` +
          `${s}${margin}-right: ${getAdaptation(array[1], important)}${important};\n` +
          `${s}${margin}-bottom: ${getAdaptation(array[2], important)}${important};\n` +
          `${s}${margin}-left: ${getAdaptation(array[1], important)}${important};`;
      case 4:
        return  `${s}${margin}-top: ${getAdaptation(array[0], important)}${important};\n` +
          `${s}${margin}-right: ${getAdaptation(array[1], important)}${important};\n` +
          `${s}${margin}-bottom: ${getAdaptation(array[2], important)}${important};\n` +
          `${s}${margin}-left: ${getAdaptation(array[3], important)}${important};`;
    }
  });

  // background
  css = css.replace(/ *background *:.+?;/g, text => text.replace(/background/, "background-color"));

  // border: none;
  css = css.replace(/ *border:\s*none;/g, text => {
    let s = text.match(/^ */);
    s = s ? s[0] : "";

    return  `${s}border-width: 0;\n` +
      `${s}border-color: none;`;
  });

  // border
  css = css.replace(/ *border(-left|-right|-top|-bottom)? *:.+?;/g, text => {
    let border = text.match(/border[-A-z]*/)[0],
      important = /!important/.test(text) ? "!important" : "",
      s = text.match(/^ */),
      array = text.replace(/ *!important/, "")
        .match(/:\s*[^;]+/);
    s = s ? s[0] : "";

    array = array[0].replace(/^:\s*/, "").match(/[\S]+/g);

    return  `${s}${border}-width: ${getAdaptation(array[0], important)}${important};\n` +
      `${s}border-style: ${array[1]};\n` +
      `${s}${border}-color: ${array[2]};`;
  });

  // text-decoration
  css = css.replace(/ *text-decoration *:.+?;/g, text => {
    let s = text.match(/^ */),
      array = text.replace(/ *!important/, "")
        .match(/:\s*[^;]+/);
    s = s ? s[0] : "";

    array = array[0].replace(/^:\s*/, "").match(/[\S]+/g);

    return  `${s}text-decoration-line: ${array[0]};\n` +
      `${s}text-decoration-color: ${array[1]};\n` +
      `${s}text-decoration-style: ${array[2]};`;
  });

  // text-shadow
  css = css.replace(/ *text-shadow *:.+?;/g, text => {
    let space = options.space,
      important = /!important/.test(text) ? "!important" : "",
      s = text.match(/ */),
      width, height, color, radio,
      textShadow = "",
      value = text.replace(/ *!important/, "")
        .replace(/^.+?:\s*/, "")
        .replace(/\s*;$/, "")
        .split(" ");
    s = s ? s[0] : "";

    if(!value || value.length < 2) return text;

    width = parseFloat(value[0]);
    height = parseFloat(value[1]);
    if(/^-?[\d.]+/.test(value[2])) {
      radio = parseFloat(value[2]);
    } else if(value[2]){
      color = value[2];
    }
    if(value[3]) color = value[3];

    textShadow += `${s}${space}textShadowOffset: {\n`;
    textShadow += `${s}${space}${space}width: ${getAdaptation(width, important)}${important},\n`;
    textShadow += `${s}${space}${space}height: ${getAdaptation(height, important)}${important}\n`;
    textShadow += `${s}}\n`;
    if(radio) textShadow += `${s}text-shadow-radio: ${radio};\n`;
    if(color) textShadow += `${s}text-shadow-color: ${color};\n`;
    textShadow = textShadow.replace(/\s+$/, "")
      .replace(/!important/g, "");

    return textShadow;
});

  // font-variant
  css = css.replace(/font-variant *:.+?;/g, text => {
    let value = text.match(/:[^;]+/, "")[0]
      .replace(/^: */, "")
      .trim().split(" ");

    value = value.filter(item => item !== "!important");
    value = value.map((item, index) =>  `"${item}"`);
    value = value.join(",");

    return `font-variant: [${value}];`;
  });

  // font
  css = css.replace(/ *font *:.+?;/g, text => {
    let important = /!important/.test(text) ? "!important" : "",
      s = text.match(/^ */),
      value = text.replace(/ *!important/, "")
        .replace(/^.+?:\s*/, "")
        .replace(/\s*;$/, ""),
      style = weight = "normal",
      size, height, family,
      variant = [],
      font = "";
    s = s ? s[0] : "";

    // font-style
    value = value.replace(/italic/, text => {
      style = text;

      return "";
    });

    // font-size  line-height
    value = value.replace(/-?[\d.]+(px|em)(\/-?[\d.]+(px|em))?/, str => {
      size = str.match(/^-?[\d.]+/)[0];
      height = str.match(/\/-?[\d.]+/);
      if(height) {
        height = height[0].replace(/^\//, "")
      }

      return "";
    });

    // font-weight
    value = value.replace(/(bold|[1-9]00)/, str => {
      weight = str;

      return "";
    });

    // font-family
    value = value.replace(/".+?"/, str => {
      family = str.replace(/"/g, "");

      return "";
    });

    // font-variant
    value = value.replace(/[A-z-,]+/, str => {
      variant = str.split(",");
      variant = variant.map(item =>  `"${item}"`);

      return "";
    });

    if(style !== "normal") font += `${s}font-style: ${style};\n`;
    if(variant.length > 0) font += `${s}font-variant: [${variant}];\n`;
    if(weight !== "normal") font += `${s}font-weight: ${weight};\n`;
    if(size) font += `${s}font-size: ${getAdaptation(size, important)}${important};\n`;
    if(height) font += `${s}line-height: ${getAdaptation(height, important)}${important};\n`;
    if(family) font += `${s}font-family: ${family};\n`;
    font = font.replace(/\s+$/, "");

    return font;
  });

  // .red
  css = css.replace(/\.[^\d][A-z\d\-_]+/g, text => {
    if(/^\.@media/.test(text)) return text;

    return options.space + text.replace(/^\./, "") + ":";
  });

  // transform
  css = css.replace(/ *transform *:.+?;/g, text => {
    let important = /!important/.test(text) ? "!important" : "",
      s = text.match(/ *transform/),
      value = text.replace(/ *transform *: */, "")
        .replace(/ *!important/, ""),
      translateX, translateY,
      scaleX, scaleY,
      rotate, rotateX, rotateY, rotateZ,
      skewX, skewY,
      transform = "[\n";
    s = s ? s[0].replace(/transform/, "") : "";
    s += options.space;

    // translate
    value.replace(/(translate|scale|rotate|skew)(X|Y|Z)?\(.+?\)/g, text => {
      let key = text.match(/(translate|scale|rotate|skew)(X|Y|Z)?/)[0],
        nums = text.match(/\(.+?\)/)[0]
          .replace(/(\(|\))/g, "")
          .split(",");

      let num1 = nums[0] ? nums[0] : "",
        num2 = nums[1] ? nums[1] : "",
        num3 = nums[2] ? nums[2] : "";

      switch(key){
        case "translate":
          translateX = getAdaptation(parseFloat(num1), important)+important;
          translateY = getAdaptation(parseFloat(num2 || num1), important)+important;
          break;
        case "translateX":
          translateX = getAdaptation(parseFloat(num1), important)+important;
          break;
        case "translateY":
          translateY = getAdaptation(parseFloat(num1), important)+important;
          break;
        case "rotate":
          rotate = `"${num1}"`;
          break;
        case "rotateX":
          rotateX = `"${num1}"`;
          break;
        case "rotateY":
          rotateY = `"${num1}"`;
          break;
        case "rotateZ":
          rotateZ = `"${num1}"`;
          break;
        case "scale":
          scaleX = getAdaptation(parseFloat(num1), important)+important;
          break;
        case "scaleX":
          scaleX = getAdaptation(parseFloat(num1), important)+important;
          break;
        case "scaleY":
          scaleY = getAdaptation(parseFloat(num1), important)+important;
          break;
        case "skew":
          skewX = `"${num1}"`;
          if(num2) skewY = `"${num2}"`;
          break;
        case "skewX":
          skewX = `"${num1}"`;
          break;
        case "skewY":
          skewY = `"${num1}"`;
          break;
      }

      return "";
    });

    if(translateX) transform += `${s}${options.space}{translateX: ${translateX}},\n`;
    if(translateY) transform += `${s}${options.space}{translateY: ${translateY}},\n`;
    if(scaleX) transform += `${s}${options.space}{scaleX: ${scaleX}},\n`;
    if(scaleY) transform += `${s}${options.space}{scaleY: ${scaleY}},\n`;
    if(rotate) transform += `${s}${options.space}{rotate: ${rotate}},\n`;
    if(rotateX) transform += `${s}${options.space}{rotateX: ${rotateX}},\n`;
    if(rotateY) transform += `${s}${options.space}{rotateY: ${rotateY}},\n`;
    if(rotateZ) transform += `${s}${options.space}{rotateZ: ${rotateZ}},\n`;
    if(skewX) transform += `${s}${options.space}{skewX: ${skewX}},\n`;
    if(skewY) transform += `${s}${options.space}{skewY: ${skewY}},\n`;

    transform += `${s}],`;
    transform = transform.replace(/!important/g, "");

    return text.replace(/transform *:.+?;/, `${options.space}transform: ${transform}`);
  });

  // "color": "red";
  css = css.replace(/[A-z\d-]+\s*:\s*.+?;/g, text => {
    return options.space + text.replace(/"/g, "'")
      .replace(/\s*:/, ":")
      .replace(/:\s*/, ": \"")
      .replace(/\s*;$/, "\",")
      .replace(/-?[\d.]+(px|em) *(!important)?/g, s => getAdaptation(parseFloat(s), /!important/.test(s)))
      .replace(/"-?[\d.]+"/g, s => s.replace(/"/g, ""))
      .replace(/"getAdaptation\(.+?\)"/g, s => s.replace(/"/g, ""))
      .replace(/(-[A-z]+)+ *:/g, s => s.replace(/-[A-z]/g, x => x.replace(/^-/, "").toUpperCase()))
      .replace(/"-?[\d.]+%? *!important"/g, s => s.replace(/"/g, "")
                                                                        .replace(/ *!important/, ""))
      .replace(/".+?!important"/g, s => s.replace(/ *!important/, ""));
  });

  css = css.replace(/}(?!,)/g, options.space + "},");

  // Multiple borderStyle
  css = css.replace(/ borderStyle:[^}]+ borderStyle:.+?\n/g, text => text.replace(/\n[^\n]+\n$/, "\n"));

  // "[]"  =>  []
  css = css.replace(/"\[.+?]"/g,
    text => text.replace(/^"/, "")
      .replace(/"$/, ""));

  // ,} => }
  css = css.replace(/,\s+?}/g, text => text.replace(/^,/, ""));

  css = getMedia(css);
  css = css.replace(/},\s+$/, "}");

  return {
    media,
    styles: css
  };
}

function getMedia(css){
  css = css.replace(/.@media[^}]+},?/g, text => {
    let key = text.replace(/^\.@media/, "")
      .match(/.+?{/)[0]
      .replace(/\s*{/, "")
      .replace(/^ ?@media/, ""),
      className,
      value = text.match(/{[\s\S]*?}/)[0];
    if(/^{[\s]+}$/.test(value)) return "";

    value = value.replace(/\s*{\s+/g, "{")
      .replace(/;\s+/g, ";")
      .replace(/,\s+/g, ",")
      .replace(/,}/g, "}")
      .replace(/\s*}\s*$/g, "}");

    className = key.match(/_\S+$/)[0]
      .replace(/^_/, "");
    key = key.replace(/_.+/, "");

    media[key] = media[key] || {};
    media[key][className] = value;

    return "";
  });

  css = css.replace(/\n\s+?\n/g, "");

  return css;
}

module.exports = CssToStyles;
